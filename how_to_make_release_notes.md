...
crew install new-release deploy-release polish-trello release-json cleanup-release
...
The release process uses five skills that chain together. Each one suggests the next step when it finishes.

```
/polish-trello → /new-release → /release-json → /deploy-release → /cleanup-release
```
...
7. Run `/cleanup-release` to tidy up branches
